...
@ensure_csrf_cookie  # Para que Django env铆e el cookie CSRF en la primera petici贸n GET si es necesario
...
@csrf_exempt  # Para desarrollo. En producci贸n, el frontend debe enviar el token CSRF.
...
def api_get_clients(request):
    # Aqu铆 deber铆as a帽adir autenticaci贸n si es necesario
...
def api_ai_search_projects(request):
    ...
    user_msg = (
        ...
        'En base a la necesidad del usuario sugiere IDs de proyectos del catalogo que est閚 relacionados con la necesidad. '
        'Responde solo con JSON {"projects": ["id1", "id2"]}. '
        'Si no hay coincidencias deja la lista vac韆.'
    )
    try:
        messages = [
    ...
    if not results:
        try:
            other_messages = [
                {
                    'role': 'system',
                    'content': 'Prop髇 un nuevo proyecto en JSON con campos name, description y technology basado en la necesidad.'
                },
                {'role': 'user', 'content': prompt},
            ]
...
def api_save_summary(request, client_id):
    if request.method == 'POST':
        try:
            data = json.loads(request.body)
            # Aqu铆 deber铆as guardar el resumen en el modelo correspondiente
            # Por ejemplo:
            cliente = get_object_or_404(Cliente, id=client_id)
